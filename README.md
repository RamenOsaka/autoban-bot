...
3. Run ```go run .``` in the terminal. The application will create ```config.json``` upon starting for the first time, this file stores every server configuration. If everything goes well, you will see ```RamenBot is now running.  Press CTRL-C to exit.```. You simply have to invite the bot to your server to use it.
